...
' ''' <summary>
' ''' The <see cref="WindowSticker"/> instance that sticks the Form on the Desktop screen borders.
' ''' </summary>
...
#Region " Event Handlers "
    ...
    ''' </summary>
    Private Sub SetFormHandle(ByVal sender As Object, ByVal e As EventArgs) _
    Handles clientForm1.HandleCreated,
            clientForm1.Load,
            clientForm1.Shown
    ...
    ''' Releases the Handle.
    ''' </summary>
    Private Sub OnHandleDestroyed(ByVal sender As Object, ByVal e As EventArgs) _
    Handles clientForm1.HandleDestroyed
...
#Region " Private Methods "
    ...
    Private Sub SnapToDesktopBorder(ByVal clientForm As Form,
        ...
        Dim newPosStruct As WindowPos = CType(Marshal.PtrToStructure(handle, GetType(WindowPos)), WindowPos)

        If (newPosStruct.Y = 0) OrElse (newPosStruct.X = 0) Then
            ' Nothing to do.
            Exit Sub
        End If

        ' Top border (check if taskbar is on top or bottom via WorkingRect.Y)
        If (newPosStruct.Y >= -snapMargin AndAlso (Me.WorkingArea.Y > 0 AndAlso newPosStruct.Y <= (Me.TaskbarHeight + Me.SnapMargin))) _
        OrElse (Me.WorkingArea.Y <= 0 AndAlso newPosStruct.Y <= (snapMargin)) Then

            If Me.TaskbarHeight > 0 Then
                ' Horizontal Taskbar
                newPosStruct.Y = Me.WorkingArea.Y
            Else
                ' Vertical Taskbar
                newPosStruct.Y = 0
            End If
        ...
        ' Left border
        If (newPosStruct.X >= Me.WorkingArea.X - Me.SnapMargin) _
        AndAlso (newPosStruct.X <= Me.WorkingArea.X + Me.SnapMargin) Then

            newPosStruct.X = Me.WorkingArea.X

        End If

        ' Right border.
        If (newPosStruct.X + clientForm.Width <= Me.WorkingArea.Right + Me.SnapMargin) _
        AndAlso (newPosStruct.X + clientForm.Width >= Me.WorkingArea.Right - Me.SnapMargin) Then

            newPosStruct.X = (Me.WorkingArea.Right - clientForm.Width)

        End If

        ' Bottom border.
        If (newPosStruct.Y + clientForm.Height <= Me.WorkingArea.Bottom + Me.SnapMargin) _
        AndAlso (newPosStruct.Y + clientForm.Height >= Me.WorkingArea.Bottom - Me.SnapMargin) Then

            newPosStruct.Y = (Me.WorkingArea.Bottom - clientForm.Height)

        End If

        ' Marshal it back.
        Marshal.StructureToPtr(structure:=newPosStruct, ptr:=handle, fDeleteOld:=True)
